...
export class Http3SpeedyRRScheme extends Http3PriorityScheme {
    private static readonly BUCKET_COUNT: number = 5;
    ...
    public constructor(logger?: QlogWrapper) {
        ...
        for (let i = 0; i < Http3SpeedyRRScheme.BUCKET_COUNT; ++i) {
            this.buckets.push(new Http3PMeenanBucket(i));
            this.bucketActivityMap.set(i, false);

            const self: Http3SpeedyRRScheme = this;
    ...
    private metadataToBucket(metadata: Http3RequestMetadata): [number, number] {
        ...
        if (metadata.mimeType.search("javascript") > -1) {
            if (metadata.isAsync === true || metadata.isDefer === true) {
                return [PriorityGroup.LOW, 1];
            }
            else if (metadata.isAfterFirstImage === true) {
                return [PriorityGroup.NORMAL, 1];
            } else {
                return [PriorityGroup.HIGH, 1];
            }
        } else if (metadata.mimeType === "text/html" || metadata.mimeType === "text/css") {
            return [PriorityGroup.HIGHEST, 1];
