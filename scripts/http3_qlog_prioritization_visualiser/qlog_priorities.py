...
for row in json["connections"][0]["events"]:
    if row[1] == "HTTP" and row[2] == "DATA_CHUNK":
        print(row)
        out.write("<div style='float:left;border:solid " + border_colours[int(row[4]["stream_id"], 16) // 4] + ";background:" + colours[int(row[4]["stream_id"], 16) // 4] + ";margin:1px;padding:5px 30px;'>ID: " + row[4]["stream_id"] + "<br/>Weight: " + str(row[4]["weight"]).zfill(3) + "<br/>Bytes:" + str(row[4]["byte_length"]) + "</div>")
